<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-900">Edit Jenjang</h2>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow sm:rounded-lg border border-gray-200">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.jenjangs.update', $jenjang) }}" class="space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="program_category_id" required
                                ...
                                @foreach($categories as $c)
                                    <option value="{{ $c->id }}"
                                        @selected(old('program_category_id', $jenjang->program_category_id)==$c->id)>
                                        {{ $c->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input name="nama" type="text" value="{{ old('nama', $jenjang->nama) }}" required
                                   class="mt-1 w-full rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="sort_order" type="number" min="0"
                                       value="{{ old('sort_order', $jenjang->sort_order) }}"
                                       class="mt-1 w-full rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <div class="flex items-end">
                                <label class="inline-flex items-center gap-2 text-gray-900">
                                    <input type="checkbox" name="is_active" value="1"
                                           ...
                                           @checked(old('is_active', (bool)$jenjang->is_active))>
