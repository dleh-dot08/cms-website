<x-app-layout>
    <x-slot name="header">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h2 class="font-semibold text-xl text-gray-900">Tag</h2>
                <p class="mt-1 text-sm text-gray-700 font-medium">Kelola tag untuk lowongan (Urgent, Remote, Fresh Graduate, dll).</p>
            </div>
                        ...
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-200 text-gray-900">
                                    ...
                                    <th class="py-3 pr-4 text-left font-bold">Slug</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse($tags as $tag)
                                    <tr class="text-gray-900">
                                        <td class="py-3 pr-4 font-semibold">{{ $tag->nama }}</td>
                                        <td class="py-3 pr-4 text-gray-800 font-medium">{{ $tag->slug }}</td>
                                        <td class="py-3 pr-4">
                                            @if($tag->is_active)
                                        ...
                                        <td class="py-3 pr-4">{{ $tag->sort_order }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="py-6 text-center text-gray-700 font-medium">
                                            Belum ada data tag.
                                        </td>
                                    </tr>
                                @endforelse
