<x-app-layout>
    <x-slot name="header">
        <div class="flex items-start justify-between gap-4">
            <div>
                <h2 class="font-semibold text-xl text-gray-900">Edit Tipe Kerja</h2>
                <p class="mt-1 text-sm text-gray-700 font-medium">{{ $workType->nama }}</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow sm:rounded-lg border border-gray-200">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.work_types.update', $workType) }}" class="space-y-5">
                        @csrf
                        @method('PUT')

                        <div>
                            <label class="block text-sm font-semibold text-gray-900">Nama Tipe Kerja</label>
                            <input type="text" name="nama" required
                                   value="{{ old('nama', $workType->nama) }}"
                        ...
                        <div>
                            ...
                            <input type="number" min="0" name="sort_order"
                                   value="{{ old('sort_order', $workType->sort_order) }}"
                                   class="mt-1 w-full rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="checkbox" name="is_active" value="1"
                                   ...
                                   @checked(old('is_active', (bool)$workType->is_active))>
                    ...
                    <div class="mt-6 border-t border-gray-200 pt-6">
                        <div class="flex items-center justify-between gap-4">
                            <div>
                                <h3 class="font-semibold text-gray-900">Hapus Tipe Kerja</h3>
                                <p class="text-sm text-gray-700">Jika dipakai oleh lowongan, data terkait ikut terhapus (cascade).</p>
                            </div>
